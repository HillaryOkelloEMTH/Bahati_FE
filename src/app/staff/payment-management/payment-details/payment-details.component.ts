...
export class PaymentDetailsComponent implements OnInit, OnDestroy {
  ...
  const sub = this.paymentService.getPaymentOptions().subscribe({
    next: (options: BankOption[]) => {
      ...
      console.log("payment options",this.paymentOptions);
  ...
  addPaymentOption(option: BankOption): void {
    ...
    const sub = this.paymentService.addPaymentOption(option).subscribe({
      ...
      error: (error) => {
        ...
        this.isLoading = false;
        this.snackbar.showNotification('snackbar-success', error.message || 'Payment option added successfully');
      }
    });
    this.subscriptions.push(sub);
  }
  ...
  toggleStatus(option: BankOption): void {
    ...
    const sub = this.paymentService.toggleStatus(updatedOption.id).subscribe({
      next: () => {
